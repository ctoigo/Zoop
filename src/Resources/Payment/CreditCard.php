 ...
 * CreditCard class
 *
 * Essa classe é responsavel por realizar o pagamento utilizando o cartão de credito, preparando e hidratando o mesmo com os dados estaticos.
 * 
...
class CreditCard extends Zoop {
     ...
     *
     * Hidrata o array basico do cartão de credito adicionando dados imutaveis para realizar a operação.
     *
     * @param array $card
    ...
    private function validateInstallment(array $installment) {
        if (!isset($installment['mode']) || !isset($installment['number_installments']) || empty($installment['mode']) || empty($installment['number_installments'])) {
            return false;
        }
     ...
     *
     * Gera o pagamento com cartão de credito, realizando com simplicidade a operação.
     *
     * @param array $card
     * @param null|string $referenceId
     *
