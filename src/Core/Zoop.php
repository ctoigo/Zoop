 ...
 * abstract Zoop class
 * 
 * Essa classe abstrata é responsavel pela magica do pacopte de pagamentos ela abstrai todos os metodos das
 * classes registradas na função getBundles, e pertime a qualquer classe que a extenda utilizar qualquer função
 * dos bundles registados, podendo assim se criar a classe Zoop\ZoopClient que realiza todas as operações do pacote.
 * 
 * @package Zoop
...
abstract class Zoop {
     ...
     * function getBundles
     * 
     * Registra todas as classes da biblioteca em um array para serem listados para utilização
     *
     * @return array
     */
     ...
     *
     * Lista todos os bundles registados para observar se não existem funções que se sobrescrevem
     * 
     * @return array
     */
     ...
     * getBundle function
     *
     * Pega o noem do Bundle dono da função que foi requisitada
     * 
     * @param array $bundles
     ...
     *
     * Lista todos os metodos que estão dentro dos Bundles registados e lista todos os bundles também para serem reutilizados.
     * 
     * @return array
     */
        ...
        foreach ($bundlesArray as $bundleKey => $bundle) {
            ...
            if (is_array($bundleMethods) && !empty($bundleMethods)) {
                foreach ($bundleMethods as $method) {
                    if ($method != '__construct' && $method != '__call' && $method != 'hookBundle' && $method != 'getAllBundle' && $method != 'getBundle' && $method != 'getBundles' && $method != 'ResponseException'
                    ) {
     ...
     * gunction __call
     * 
     * Pega metodos que estão fora dessa classe porem que pertencem ao namespace e estão
     * registados no registro de bundles acima e utiliza de funções especiais para habiliar
