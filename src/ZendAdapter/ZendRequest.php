 ...
 * ZendRequest class
 * 
 * Essa classe realiza em formato Fluente assum como o Guzzle a adaptação das requests realizadas com Guzzle para o Zend Http
...
class ZendRequest {
     ...
     * getConfig function
     *
     * Retorna as configurações setadas pelo usuario via injeção de dependencia.
     * 
     * @return array|void
     */
    ...
    public function request($method, $url, $body = null, $headers = null) {
        ...
        if ($method == 'GET' || $method == 'POST') {
            $this->request = ClientStatic::$method($this->guzzle['base_uri'] . $url, $body, $headers);
        } else {
            $request = new Request();
     ...
     *
     * Pega todos os dados fluidos realiza as verificações bases, como se a zoop retornou algum error, se sim cria a exception
     *
     * @return string
     * @throws \Exception
     */
